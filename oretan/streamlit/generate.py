...
# Akan membuat folder data/ berisi semua CSV dummy
...
for i in range(1, N + 1):
    ...
    usia = int(rng.integers(batas_usia + 1, batas_usia + 5)) \
           if rng.random() < 0.15 else int(rng.integers(21, batas_usia + 1))

    ipk = round(float(rng.uniform(2.50, 2.99)), 2) \
          if rng.random() < 0.12 else round(float(rng.uniform(3.00, 4.00)), 2)

    ukbi = int(rng.integers(500, 578)) \
           if rng.random() < 0.10 else int(rng.integers(578, 701))

    jenis_tes = str(rng.choice(jenis_tes_list))
    batas_min = batas_bahasa[jenis_tes]
    if rng.random() < 0.10:
        skor_tes = round(float(rng.uniform(4.0, 6.4)), 1) \
                   if jenis_tes == 'IELTS' else int(rng.integers(int(batas_min * 0.8), batas_min))
    else:
        skor_tes = round(float(rng.uniform(6.5, 9.0)), 1) \
                   if jenis_tes == 'IELTS' else int(rng.integers(batas_min, int(batas_min * 1.3)))
...
semua_ids = df['id'].tolist()
for folder, kriterias in KRITERIA_FOLDER.items():
    for k in kriterias:
        nilai = np.random.uniform(1, 10, (len(semua_ids), 10)).round(2)
        df_n  = pd.DataFrame(nilai, index=semua_ids,
                             columns=[f'Asesor_{j+1}' for j in range(10)])
        df_n.index.name = 'id'
        df_n.to_csv(f'{NILAI_DIR}/{folder}/{k}.csv')
    print(f'✅ {folder}: {len(kriterias)} file kriteria tersimpan')

print('\n✅ Semua data dummy siap di folder data/')